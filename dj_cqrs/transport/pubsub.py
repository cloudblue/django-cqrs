...
class PubSubTransport(LoggingMixin, BaseTransport):
    ...
    def _produce(cls, payload):
        ...
        cls._produce_message(connection, topic, payload)
    ...
    #     cls._produce_message(connection, exchange, payload, expiration)
    ...
    @classmethod
    def _produce_message(cls, connection, topic_name, payload, expiration=None):
    ...
    @classmethod
    def _get_consumer_pubsub_objects(
        cls,
        host,
        port,
        creds,
        ...
        # # cls._declare_topic(channel, exchange)
    ...
    @classmethod
    def _get_producer_pubsub_objects(cls, host, port, creds, exchange, signal_type=None):
        """
        ...
        if signal_type == SignalType.SYNC:
            if cls._producer_connection is None:
                connection = cls._create_connection(host, port, creds, exchange)
                cls._producer_connection = connection
            return cls._producer_connection
        else:
    ...
    #     topic = settings.CQRS.get('topic', 'cqrs')
    #     return (
    #         host,
    #         port,
    ...
    #         dead_letter_topic_name = replica_settings['dead_letter_topic']
